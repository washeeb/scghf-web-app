...
| 16 | **Should a donor be able to turn on two-factor authentication?** Blueprint §7.1 describes it as "optional (encouraged)" for donors, and there is currently no way for one to enable it — `UserType::requiresTwoFactor()` returns false for donors and no page offers enrolment. That is an honest absence rather than a gap: nothing in the code claims donors have it. But the columns, the encrypted casts and Filament's TOTP contracts are all already on `User`, so building it is enrolment UI plus a challenge step at sign-in rather than new infrastructure. The question is whether the audience warrants it — a donor account holds a giving history, not the ability to move money. | whether Phase 4 or a later phase builds donor TOTP enrolment |
| 17 | **How does a donor change the email address on their account?** Deliberately not self-service. Changing the address is how a stolen session becomes a permanent takeover, because password resets then go to an inbox the attacker controls — doing it safely needs the current password, a confirmation link to the new address and a warning to the old one, which is a security flow rather than a profile field. The profile page says to write in, and staff will change it through the Filament user resource in Phase 5. Confirm that is acceptable operationally, or the self-service flow gets built. | Phase 5's admin user resource; the wording on `/account/profile` |
