...
| **Version** | 1.3 — docroot confirmed; hosting & credentials decisions recorded |
...
### Paystack — pending, placeholdered

The merchant account is not open yet. Split into two kinds of value:

**Non-secret facts → CMS settings, safe to record here:**

| Token | What it is | Your value |
|---|---|---|
| `{{PAYSTACK_BUSINESS_NAME}}` | Name shown on the hosted checkout page and on the donor's statement | |
| `{{PAYSTACK_SUPPORT_EMAIL}}` | Support address Paystack displays to payers | |
| `{{PAYSTACK_MERCHANT_ID}}` | Merchant identifier (not a secret) | |
| `{{PAYSTACK_SETTLEMENT_BANK}}` · `{{PAYSTACK_SETTLEMENT_ACCOUNT}}` | Where payouts land — for the reconciliation module | |
| `{{PAYSTACK_CHANNELS}}` | *(default: `mobile_money`, `card`)* | |

**Secret values → `.env` only, never here:**

`PAYSTACK_PUBLIC_KEY` · `PAYSTACK_SECRET_KEY` · `PAYSTACK_WEBHOOK_SECRET`

> These three live **only** in `/home/presti98/scghf/shared/.env`, documented in `.env.example` with obviously-fake values (`pk_test_xxxxxxxxxxxx`). They must never appear in this file, in the repository, in a GitHub secret, or in any chat. **See §11.6.2 — the payments module is built and fully tested without them.**
...
| `{{ADDON_DOCROOT}}` | Document root of the addon domain | **`/home/presti98/greaterhopefoundations.com`** | ✅ confirmed |
...
| `{{SMS_PROVIDER}}` | Arkesel / Hubtel / mNotify — **placeholdered**, driver-agnostic (§11.6.3) | | when ready |
| `{{SMS_SENDER_ID}}` | *(suggested: `GreaterHOPE`, 11 chars max)* — **placeholdered** | | when registered |
| `{{SMS_API_KEY}}` | **`.env` only**, never in this document | | when ready |
...
| ~~The addon domain's document root~~ | ✅ **Resolved — `/home/presti98/greaterhopefoundations.com`** | Release layout in §11.3 is now fully literal |
...
**One important correction from v1.0:** `greaterhopefoundations.com` is an **addon domain**, so `public_html` belongs to `prestigerocktravels.com` — **we must not touch it.** Our document root is confirmed as:

```
/home/presti98/greaterhopefoundations.com
```

Every path below is now literal. Nothing in the deployment plan is still a guess.

```
/home/presti98/
├── public_html/                        ← prestigerocktravels.com — DO NOT TOUCH
│
├── greaterhopefoundations.com/         ← OUR DOCUMENT ROOT
...
**The docroot swap, done safely.** `greaterhopefoundations.com` already exists as a real directory, so the first deploy replaces it with a symlink. Back it up first — this is the one irreversible-looking step in the whole setup:

```bash
mv /home/presti98/greaterhopefoundations.com /home/presti98/greaterhopefoundations.com.bak
ln -s /home/presti98/scghf/current/public /home/presti98/greaterhopefoundations.com
```

If anything is wrong, `rm` the symlink and `mv` the `.bak` directory back. Keep the backup until the site has served real traffic for a week. ⚠️ If Apache is configured with `FollowSymLinks` disabled for addon domains, this returns 403 — that is the trigger for the thin-`index.php` fallback noted in the tree above, not a reason to move code into the web root.
...
| 2 | **Create the Paystack merchant account** in the foundation's name with its registration documents; complete KYC; obtain **test and live** keys — *placeholdered, does not block the build (§11.6.2)* | **3–10 working days.** Needs the registration certificate first. |
| 3 | **Open the SMS provider account** (Arkesel / Hubtel / mNotify) and **submit the alphanumeric sender ID for registration** — e.g. `GreaterHOPE` (11 chars max) — *placeholdered, does not block the build (§11.6.3)* | **1–5 working days after** the registration certificate. **The longest pole in the launch schedule** (risk DEL-6). |
...
## 11.6 Building around what isn't ready yet

Three decisions have been made: **stay on the shared account for now**, **Paystack placeholdered**, **SMS placeholdered**. None of them block the build — but each has an architectural consequence that must be designed in from Phase 2, not retrofitted.
...
| Rule | Why it makes the move cheap |
|---|---|
| **Nothing host-specific in the repository.** Every absolute path, username, DB name, hostname and port comes from `.env` or a CI secret — never hardcoded, never in a migration or seeder. | Moving hosts becomes a `.env` edit plus new GitHub secrets. Zero code changes. |
| **Deploy paths derive from one variable.** `DEPLOY_PATH=/home/presti98/scghf` is a single CI secret; everything else is relative to it. | One secret to change. |
| **Media on a documented, portable path** — `storage/app/public`, never scattered — with the object-storage escape hatch (risk SH-6) designed in from the start via Laravel's filesystem disks. | Switching to R2/B2 later is a `FILESYSTEM_DISK` change, and it makes the host move nearly stateless. |
| **Database dumps are part of the routine, not an emergency skill.** `spatie/laravel-backup` writes off-server nightly from day one. | The move is: restore the latest backup on the new host, repoint DNS. Already rehearsed. |
| **DNS TTL lowered to 300s a week before any move.** | Cutover measured in minutes, not a day of split-brain traffic where some donors hit the old server. |

**Two things to do now, while it costs nothing:**

1. **Register the domain to the foundation, not to an individual or to the travel business.** Hosting can move later; a domain in the wrong name is a much harder conversation. *(This is separate from where the site is hosted.)*
2. **Do not let donor data accumulate on this account longer than necessary.** The move is cheapest before there are real donors, and the governance argument (OPS-11) gets sharper with every record. My recommendation stands: **move before the first live donation** if you can — but the build proceeds either way.

**Trigger for the move — pick one now so it doesn't drift:** the first live donation · 100 donor records · the first institutional-donor due-diligence request · or a fixed date. Whichever comes first.

### 11.6.2 Payments: the module is built and fully tested without Paystack credentials

`{{PAYSTACK_*}}` being empty does **not** delay Phase 8. It only delays the final live smoke test.

| Layer | Built without credentials? | How |
|---|---|---|
| Money handling — pesewa integers, `Money` cast, `GH₵` formatting | ✅ Entirely | Pure domain logic. Pest tests at boundary values. Nothing touches Paystack. |
| Donation/order records, references, designation, fee-cover, tribute gifts | ✅ Entirely | Database + application logic |
| `PaystackService` API calls | ✅ Fully testable | `Http::fake()` with recorded fixtures of Paystack's documented response shapes |
| **Webhook signature verification** | ✅ Fully testable | Generate the HMAC locally with a dummy secret, assert valid signatures pass and tampered ones return 401. **This needs no account at all.** |
| **Idempotency / replay protection** | ✅ Fully testable | Fire the same fixture event 5× and assert one donation, one receipt |
| Amount/currency re-verification, `needs_review` path | ✅ Fully testable | Feed a mismatched fixture and assert it does *not* auto-complete |
| Live checkout, real MoMo prompt, real settlement | ❌ Needs the account | Phase 8 go-live smoke test only |

**Architecture that makes this true:** a `PaymentGateway` interface with two implementations — `PaystackGateway` (real) and `FakeGateway` (local + CI, returns deterministic fixtures). `PAYMENT_DRIVER=fake` locally, `paystack` on staging and production. **Controllers and Livewire components depend on the interface, never on Paystack directly** — which is the same rule `CLAUDE.md` already sets, now earning its keep.

> **The moment the account exists**, the only change is filling three `.env` values on staging and running the live smoke test. No code changes. If you get the account before Phase 8 lands, even better — but nothing waits on it.

### 11.6.3 SMS: driver-agnostic from day one

You have not picked a provider and the sender ID is not registered. So we do not pick one either — we build to an interface.

- `SmsGateway` interface, with implementations added as needed: `ArkeselDriver`, `HubtelDriver`, `MNotifyDriver`, plus **`LogDriver` as the default**.
- **`SMS_DRIVER=log` until credentials arrive.** Every SMS is written to the `sms_logs` table exactly as if it had been sent — right recipient, right rendered template, right segment count and cost estimate — but nothing leaves the server. The whole notification flow is therefore end-to-end testable today, and the admin can see precisely what *would* have gone out.
- `{{SMS_SENDER_ID}}` is a **settings value**, so registering `GreaterHOPE` later is a Filament form edit, not a deploy.
- Switching provider later is one `.env` line, because nothing above the interface knows which provider is behind it.

> ⚠️ **The one thing that cannot be placeholdered away: the registration lead time.** An unregistered alphanumeric sender ID is *silently dropped* by MTN, Telecel and AT — no bounce, no error, the donor simply never gets the receipt. Registration takes 1–5 working days and needs a business registration document, which you also do not have yet. **This is now the longest pole in the launch schedule** — longer than any code. Start it the day the registration certificate is in hand. Until it is approved, `SMS_DRIVER` falls back to the provider's shared sender ID rather than to silence.

### 11.6.4 A go-live gate for the placeholders

Because `{{TOKENS}}` will be filled in over weeks by different people, we need something that answers "what is still fake?" without anyone having to remember. Phase 4 ships:

```bash
php artisan scghf:preflight
```

It reports, grouped by severity, every setting still holding a placeholder or default, and every one that must be real before launch:

- 🔴 **Blocks launch** — legal name, registration number, contact address, phone, donation email, bank details, live Paystack keys, `APP_DEBUG=false`, `APP_ENV=production`, a registered SMS sender ID, SPF/DKIM/DMARC present, `/​.env` returning 403
- 🟡 **Should be filled** — board members, social handles, partner logos, annual report, office hours
- 🟢 **Optional** — secondary phone, TikTok, SWIFT code

The same command runs in CI against staging, and **production refuses to boot with `sk_test_` keys or an unfilled legal name**. That is what makes "use placeholders and fill them later" a safe strategy rather than a way to quietly launch a half-configured donation site.
...
| 3 | ~~Is the Paystack merchant account open?~~ ✅ **Decided — placeholdered.** The payments module is built and fully tested without it (§11.6.2). Start the application once the registration certificate is in hand; it is not on the critical path until Phase 8's go-live smoke test. |
...
| 13 | ~~SMS provider~~ ✅ **Decided — placeholdered.** Built driver-agnostic with `SMS_DRIVER=log` until credentials exist (§11.6.3), so the choice can wait. **Recommend Arkesel or Hubtel** when you do choose. ⚠️ The *sender-ID registration* lead time cannot be placeholdered away — it is the longest pole in the launch schedule. |
...
*End of Phase 1 Blueprint · St. Cecilia's Greater Hope Foundations · v1.3*
