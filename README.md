...
**Live chat and courier deliveries (21 September 2026)** — a live chat between visitors and the office, held on this server and answered from Inbox → Live chat (`FEATURE_LIVE_CHAT`; every word under Settings → Live chat; transcripts; twelve-month retention); and deliveries by the foundation's own riders — the office assigns a courier on the order, the rider confirms pick-up, out-for-delivery and the delivery with proof from `/courier` on their phone, and the order and the customer follow. Every word of the header and footer chrome is a setting. See `CHANGELOG.md` and manual chapters 12 and 13.

**The hero rotates, and an assistant answers the chat (25 September 2026)** — the hero block takes extra slides and a height an editor chooses (compact by default, which is about two-thirds of what it was), with a carousel that honours `prefers-reduced-motion`, stops when touched, and keeps off-screen slides out of the tab order. And a live chat can be answered first by an automated assistant: it draws only on the foundation's own published pages and FAQs, tells the visitor plainly what it is, offers a person at all times, and is refused outright — in code, before any model is called — from anything about a child's safety, an emergency, a particular donation or order, a complaint or anything legal, each of which routes to the contact department that owns it. `FEATURE_CHAT_AGENT` plus an API key, off by default, with a monthly ceiling in cedis. WhatsApp messages arrive in the same inbox. See manual chapter 14.

**The template and the logo (20 September 2026)** — the site takes its shape from the foundation's chosen template (serif display titles, eyebrows, pill buttons, brand bands, the hero's rising panel) and its logo from the uploaded pack, imported by `scghf:brand-assets` on every deploy (`resources/brand/`). See `CHANGELOG.md` and manual chapters 2 and 8.

**Launch content in place (20 September 2026)** — the foundation's own words on every page, placeholder programmes and appeals marked for replacement, and 47 licensed photographs fetched by `scghf:launch-images`. See `CHANGELOG.md` and manual chapter 4.
