...
| `{{SSH_HOST}}` · `{{SSH_PORT}}` | For the deploy pipeline | | ⚠️ from InMotion |
...
| **Shared IP** | `23.235.219.254` | Shared, so email reputation risk DEL-3 / SH-14 stands |
...
The two screenshots closed most of the gaps. Six remain:

| Unknown | How to get it | Why it matters |
|---|---|---|
| **Is SSH actually enabled on this plan?** And the hostname + port | SSH Access → add a key, then connect | **Decides the deployment strategy** (§11.2). The single most important remaining item. |
...
**Both A and B need SSH.** If InMotion will not enable SSH on this plan, we fall back to B *with* committed build artefacts, and the first recommendation becomes upgrading the plan or moving to a host that allows SSH.
...
| 2 | **Enable SSH access** and add the deploy public key I will give you | Security → SSH Access |
...
| 3 | "Please confirm **SSH access** is enabled for account `presti98`, and give me the **hostname and port**." | **Decides deployment strategy A vs B — the top remaining item** |
...
| 2 | **Is SSH enabled for `presti98`?** Plus the hostname and port. This is now the *only* remaining item that can change the deployment strategy — everything else about the environment is settled. |
...
| 7 | **SSH works** | Add your key in SSH Access, then `ssh -p <port> user@host` from your machine | You get a shell prompt. **This confirms deployment strategy A.** |
...
*End of Phase 1 Blueprint · St. Cecilia's Greater Hope Foundations · v1.1*